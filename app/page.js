...
export default function PlayerPage(){
  ...
  useEffect(()=>{const p=new URLSearchParams(location.search);const code=(p.get('room')||'').toUpperCase();if(code){setRoomCode(code);try{const saved=JSON.parse(localStorage.getItem(`primos_player_${code}`)||'null');if(saved?.id&&saved?.token){setPlayer(saved);setName(saved.name||'Manager');}}catch{}}},[]);
  async function fetchState(){if(!roomCode)return;try{const r=await fetch(`/api/room/state?room=${encodeURIComponent(roomCode)}`,{cache:'no-store'});const d=await r.json();if(d.ok)setState(d);}catch{}}
  ...
  useEffect(()=>{if(!player||!state?.players||state?.room?.status!=='lobby')return;const stillInRoom=state.players.some(p=>p.id===player.id);if(!stillInRoom){localStorage.removeItem(`primos_player_${roomCode}`);setPlayer(null);setState(null);setName('');setAnswerResult(null);setAnsweredQuestion(null);setError('');}},[state,player,roomCode]);

  async function join(e){e.preventDefault();setError('');const r=await fetch('/api/room/join',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({roomCode,name})});const d=await r.json();if(!d.ok){setError(d.error||'Could not join.');return;}setPlayer(d.player);setName(d.player.name);localStorage.setItem(`primos_player_${roomCode}`,JSON.stringify(d.player));await fetchState();}
  async function answer(index){if(!state?.question||answerResult)return;const q=state.question.index;setAnsweredQuestion(q);setError('');const r=await fetch('/api/answer',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({roomCode,playerId:player.id,playerToken:player.token,questionIndex:q,answerIndex:index})});const d=await r.json();if(!d.ok){setError(d.error||'Could not submit answer.');await fetchState();return;}setAnswerResult({...d,chosen:index});await fetchState();}
  ...
  if(!player){return <main className="page"><div className="hero card"><div className="top"><h1>🏇 Primos Trivia Derby</h1><p>Join the room, answer fast, move your horse.</p></div><form className="stack" onSubmit={join}><label><span className="label">Room code</span><input className="input" value={roomCode} onChange={e=>setRoomCode(e.target.value.toUpperCase())} maxLength={10} placeholder="PRIMOS26" /></label><label><span className="label">Manager name</span><input className="input" value={name} onChange={e=>setName(e.target.value)} maxLength={24} placeholder="Your name" /></label>{error&&<div className="error">{error}</div>}<button className="btn primary" type="submit">Join Race</button><a className="btn center" href="/host" style={{textDecoration:'none'}}>Host / TV Screen</a></form></div></main>;}

  return <main className="page"><div className="shell"><div className="top"><h1>🏇 {name}</h1><p>Room <strong>{roomCode}</strong>{state?.room?.difficulty?` · ${state.room.difficulty.toUpperCase()}`:''}</p></div>
    <div className="grid playerGrid">
      <section className="card"><div className="status">YOUR HORSE</div><div className="question">{me?.finish_place?`Finished — Draft Pick #${me.finish_place}`:`${me?.yards||0} yards`}</div><div className="lane soloLane"><div className={`horse${me?.finish_place?' finishedHorse':''}`} style={{left:me?.finish_place?'91%':`${Math.min(88,2+(me?.yards||0)*.86)}%`}}><span className="horseIcon">🐎</span></div></div><div className="spacer"/><div className="muted">Correct: +10 yards. 🔥 Double Yards questions are +20. Wrong: −3 yards. No timer — answer as fast as you can.</div></section>
