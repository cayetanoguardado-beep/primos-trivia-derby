...
create or replace function public.apply_answer_v2(
  p_room_code text,
  p_player_id uuid,
  p_question_index integer,
  p_answer_index integer,
  p_correct boolean,
  p_correct_yards integer
)
returns jsonb
language plpgsql
security definer
...
begin
  select current_question, status into active_question, room_status
  from rooms where code=p_room_code for update;
  ...
  if exists (select 1 from answers where room_code=p_room_code and player_id=p_player_id and question_index=p_question_index) then
    raise exception 'duplicate answer';
  end if;

  select yards into old_yards from players where id=p_player_id and room_code=p_room_code for update;
  if old_yards is null then raise exception 'player not found'; end if;

  awarded := case when p_correct then greatest(1,least(50,p_correct_yards)) else -3 end;
  ...
  if new_yards >= 100 then
    ...
    if place is null then
      select coalesce(max(finish_place),0)+1 into place from players where room_code=p_room_code;
  ...
  if p_correct then
    select count(*) into unfinished from players where room_code=p_room_code and finish_place is null;
    if unfinished=0 then
      race_finished:=true;
      update rooms set status='finished',question_started_at=null,updated_at=now() where code=p_room_code;
    else
      update rooms set current_question=current_question+1,question_started_at=now(),updated_at=now() where code=p_room_code;
    end if;
  end if;

  return jsonb_build_object('awardedYards',awarded,'speedBonus',0,'yards',new_yards,'finishPlace',place,'advanced',p_correct,'finished',race_finished);
end;
$$;
